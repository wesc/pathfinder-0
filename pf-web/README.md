...
* [`src/pf_web/`](src/pf_web/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/pf_web/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `pf-web` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/pf_web/config.cljs) defaults to `true` in
...
```clj
(ns pf-web.example
  (:require [pf-web.config :as config])
...
- Pathfinder principles:

  1. explore new territory (bubble graph is novel, different than regular DAG view)
  2. explore workflows (diff, using github as collaborative system w/ comments)
  3. explore catalog (searchable subtrees)
  4. link to funding mechanisms such as devpost

Backend is written such that it could be sync'd over IPFS.


#### Features

- save to memory when navigating around

- link subtrees

- add profile icon to comments


- show some help text for buttons

- devpost link or "funding level"

- ability to resolve comments

- searchable catalog

- periodic catalog refreshing (so comments appear) -- do this in a janky way

- input example subtree

- add long/short desc to subtree

- publish / diff / merge subtree from remote source

- expanded tech graph view


#### Fixes

- note public repository of subtrees

- /api/catalog returns *everything*

- fix selection of default tech

- put subtree meta into its own tab

- mobile friendly

- when mousing over a tech, highlight its dependencies and dependents.

- add undo/redo on db

- show better "no tech selected" message

- prevent adding circular dependencies
